...
address, mail sent there will be forwarded to the same address without dots in the local name.  For example, "alice.z@leetcode.com" and "alicez@leetcode.com" forward to the same email address.  
...
class Solution:
    def numUniqueEmails(self, emails):
        ...
        res = []
        for item in emails:
            fore, last = item.split("@")
            if "+" in fore:
                fore, *xy = fore.split("+")
            fore_res = fore.replace('.', '')
            st_temp = fore_res + last
            if st_temp not in res:
                res.append(st_temp)
        return len(res) 